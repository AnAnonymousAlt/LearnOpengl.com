...
enum Uniform_ID
{
	Ucrimson, UsContainer, UsFace, UtransMat, numUniforms
};
...
init ()
{
	...
	uniforms[UsFace] = glGetUniformLocation ( programs[Program0],
											  "sFace" );
	uniforms[UtransMat] = glGetUniformLocation ( programs[Program0],
												 "transMat" );
...
display ()
{
	...
	glm::vec3 move ( 0.34f, 0.0f, 0.0f );
	transMat = glm::translate ( transMat, glm::vec3 ( 0.0f ) );
	transMat = glm::translate ( transMat, move );
	transMat = glm::rotate ( transMat, glm::radians ( (float)glfwGetTime() * 360 ),
							 glm::vec3 ( 0.0f, 0.0f, 1.0f ) );
	glClear ( GL_COLOR_BUFFER_BIT );
	glBindVertexArray ( VAOs[Triangle] );
	glUseProgram ( programs[Program0] );
	glUniformRGBA ( uniforms[Ucrimson], Color::crimson );
	glUniformMatrix4fv ( uniforms[UtransMat], 1, GL_FALSE,
